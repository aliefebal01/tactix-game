...
from scripts_1.dqn_agent_mh_attention import DQNAgent
...
class TrainAndPlot_opp_trans_mh_attention:
    def __init__(self,
                env_size = 5,
                n_episodes=100000, 
                max_t=1000, 
        ...
        for episode in progress_bar:
            ...
            while not done:
                ...
                if self.env.game.current_player == -1:
                    ...
                    state = torch.from_numpy(np.array(self.env.game.getPieces(), dtype=np.float32))  #for adding it to replay memory
                    curr_node = MCTSNode(self.env.game)
                    mcts_best_node = mcts_agent.best_child(curr_node)

                    action = encode_action(mcts_best_node.action_got_here, self.env.game.height)
                    
                    self.env.game = mcts_best_node.state
                    next_state = torch.from_numpy(np.array(self.env.game.getPieces(), dtype=np.float32)) #for adding it to replay memory
                    ...
                    if game_ended and game_ended.is_ended:
                        ...
                        reward = -1
                        #print('MCTS lost')
                    elif np.sum(self.env.game.getPieces()) == 1:
                        reward = 1
                    else:
                        reward = 0
    ...
    def run_experiments(self, pretrained_model_path=None):
        ...
        save_dir = os.path.join(base_dir,f"training_results_{self.env.game.height}x{self.env.game.height}_opp_trans_mh_attention")
